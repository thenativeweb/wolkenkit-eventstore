...
const pre = function (done) {
  shell.exec('docker run -d -p 27019:27017 --name mongodb-integration mongo:3.4.2', errMongo => {
    ...
    shell.exec('docker run -d -p 5434:5432 -e POSTGRES_USER=wolkenkit -e POSTGRES_PASSWORD=wolkenkit -e POSTGRES_DB=wolkenkit --name postgres-integration postgres:9.6.2-alpine', errPostgres => {
      if (errPostgres) {
