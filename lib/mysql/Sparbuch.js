...
class Sparbuch extends EventEmitter {
  ...
  async initialize ({ url, namespace }) {
    ...
    const query = `
      ...
      RETURNS BINARY(36)
      RETURN LCASE(CONCAT_WS('-', HEX(SUBSTR(_bin,  5, 4)), HEX(SUBSTR(_bin,  3, 2)), HEX(SUBSTR(_bin,  1, 2)), HEX(SUBSTR(_bin,  9, 2)), HEX(SUBSTR(_bin, 11))));
  ...
  async saveEvents ({ events }) {
    ...
    try {
      for (let i = 0; i < events.length; i++) {
        ...
        const [ rows ] = await connection.execute('SELECT LAST_INSERT_ID();');

        events[i].metadata.position = Number(rows[0]['LAST_INSERT_ID()']);
      }
