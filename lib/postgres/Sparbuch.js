...
Sparbuch.prototype.getEventStream = function (aggregateId, options, callback) {
  ...
  this.getDatabase((errGetDatabase, database, close) => {
    ...
    const eventStream = database.query({
      ...
      text: `
        SELECT "event", "position"
          ...
          ORDER BY "revision"
      `,
      values: [ aggregateId, options.fromRevision, options.toRevision ]
    });
    ...
    const unsubscribe = function () {
      close();
      eventStream.removeListener('row', onRow);
...
Sparbuch.prototype.getUnpublishedEventStream = function (callback) {
  ...
  this.getDatabase((errGetDatabase, database, close) => {
    ...
    const passThrough = new PassThrough({ objectMode: true });
    const eventStream = database.query({
      name: 'get unpublished event stream',
      text: `
        SELECT "event", "position"
          FROM "${this.namespace}_events"
    ...
    const eventStream = database.query({
      ...
      text: `
        SELECT "event", "position"
          ...
          ORDER BY "position"
      `,
      values: [ options.fromPosition, options.toPosition ]
    });
