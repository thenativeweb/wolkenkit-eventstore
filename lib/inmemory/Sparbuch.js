...
class Sparbuch extends EventEmitter {
  ...
  async initialize ({ url, namespace }) {
    ...
    // Simulate the disconnection directly after the first "connection"
    setImmediate(() => {
      this.emit('disconnect');
    });

    // Feels a bit useless because there is no situation where the inmemory db is not reachable
    if (url === '/dev/null') {
